...
def all_teachers(request):
    ...
    return render(
        request, "teacher/all_teachers.html", {"teachers_list": teachers_list}
    )
...
def edit_teacher(request, teacher_id):
    try:
        ...
        messages.error(
            request, f"Teacher with ID: {teacher_id} doesn't exist. Try another ID!"
        )
        return HttpResponseRedirect(reverse("all_teachers"))
    if request.method == "POST":
        if "edit" in request.POST:
            ...
            if not form.is_valid():
                ...
                return HttpResponseRedirect(
                    reverse("edit_teacher", args=[form.instance.id])
                )
            form.save()
            messages.success(
                request, f"Teacher with ID {teacher_id} update successful!"
            )
            return HttpResponseRedirect(reverse("all_teachers"))
...
def all_subjects(request):
    ...
    return render(
        request, "teacher/all_subjects.html", {"subjects_list": subjects_list}
    )
...
def edit_subject(request, subject_id):
    try:
        ...
        messages.error(
            request, f"Teacher with ID: {subject_id} doesn't exist. Try another ID!"
        )
        return HttpResponseRedirect(reverse("all_subjects"))
    if request.method == "POST":
        if "edit" in request.POST:
            ...
            if not form.is_valid():
                messages.error(request, "Form isn't valid. Try again!")
                return HttpResponseRedirect(
                    reverse("edit_subject", args=[form.instance.id])
                )
            form.save()
            messages.success(
                request, f"Subject with ID {subject_id} update successful!"
            )
            return HttpResponseRedirect(reverse("all_subjects"))
        elif "delete" in request.POST:
            subject.delete()
